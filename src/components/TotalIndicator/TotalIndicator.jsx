...
export default function TotalIndicator({ title, value, collation, waves }) {
    ...
    const [collationColor] = React.useState(collationValue >= 0 ? "#34e4b5" : "#E03A45");
    const [collationIcon] = React.useState(collationValue > 0 ? "/img/arrow_up.svg" : "/img/arrow_down.svg");
    const [wavesSrc] = React.useState(waves || "/img/waves.svg");
	...
	return (
		<div className={css.ti__wrapper}>
			<div className={css.ti__textRows}>
				...
				<p className={css.ti__collation} style={{color: collationColor}}>
                    <img src={collationIcon} />
                    {collationValue}%</p>
			</div>
